...
import { ClusterOutlined, HomeOutlined, CarOutlined, UserOutlined } from '@ant-design/icons';
    ...
    { text: '사건장소는 어떻게 입력해야 하나요?', type: 'sent' },
    {
      text: (
        <>
          ...
          장소의 카테고리를 입력하는 게 좋습니다.<br />
          예를 들어, OO기업(X) ⭢ 회사(O)<br />
          OO아파트(X) ⭢ 아파트(O)
        </>
      ),
      type: 'received',
    ...
    { text: '사건대상은 어떻게 입력해야 하나요?', type: 'sent' },
    {
      text: (
        <>
          ...
          실제 대상명 보다 대상의 특성 또는 대상의 카테고리를 입력하는 것이 좋습니다.<br />
          이를 통해 추천 검색 결과의 정확도를 높일 수 있습니다.<br />
          예를 들어, OO로 OO(X) ⭢ 토지(O)<br />
          OO차,OO원(X) ⭢ 자동차, 자산(O)
        </>
      ),
      type: 'received',
  ...
  const personMessages: Message[] = [
    { text: '사건인물은 어떻게 입력해야 하나요?', type: 'sent' },
    {
      text: (
        <>
          6자 이상 입력해주세요.<br />
          예: 채무자, 채권자, 근로자, 참가인, 조합원, 피해자, 소유자 등 인물의 실명보다 인물의 역할, 관계를 입력하는 것이 좋습니다.<br />
          이를 통해 추천 검색 결과의 정확도를 높일 수 있습니다.<br />
          예를 들어, 김OO(X) ⭢ 채무자(O)<br />
          황OO씨(X) ⭢ 피해자(O)
        </>
      ),
      type: 'received',
  ...
  return (
    <div className={style.recommendSearchContainer}>
      ...
      <div className={style.searchContainer}>
        ...
        {currentStep > 4 && (
          <div className={`${style.searchItem} ${style.fadeInUp}`}>
            <Button type="primary" onClick={handleSubmit} style={{ width: 100, height: 50, fontSize: 18, marginLeft: '39%' }}>제출</Button>
          </div>
        )}
      </div>
    </div>
