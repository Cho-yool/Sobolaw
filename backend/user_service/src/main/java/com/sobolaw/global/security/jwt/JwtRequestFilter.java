...
//        log.info("예외처리 주소 : " + servletPath.contains("/api/user-service/swagger-ui/index.html"));
        if (
            servletPath.contains("swaggers") || servletPath.startsWith("/oauth2") || servletPath.startsWith("/login") || servletPath.startsWith("/token/refresh") || servletPath.startsWith("/swagger-ui/index.html")
        ) {
            log.info("토큰 검사 pass");
